...
export function ApiDataGrid({ metrics, error }) {
  ...
  const columns = [
    ...
    {
      field: 'availability_uptime',
      headerName: 'Availability (%)',
      width: 150,
    },
    {
      field: 'last_Error_Occurred',
      headerName: 'Last Error',
      width: 250,
      renderCell: (params) => {
        return params?.row?.last_Error_Occurred
          ? moment(params?.row?.last_Error_Occurred).format(
              'MMMM Do YYYY, h:mm:ss a'
            )
          : 'Never Failed Since Scheduled';
      },
    },
    { field: 'avg_latency', headerName: 'Avg Latency(sec)', width: 150 },
    {
      ...
      renderCell: (params) => {
        ...
        const handleToggle = () => {
          ...
          console.log(
            `Toggled status for API ${params.row.apiName}: ${!isActive ? 'Active' : 'Inactive'}`
          );
        };
