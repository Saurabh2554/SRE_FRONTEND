  ...
  const addHeaderFields = () => {
    ...
    if(state.authorizationType === 'API_KEY' && (state.authInput.username !== '' && state.authInput.password !== '') ){
      ...
      if(state.authInput.username && state.authInput.password){
        const encodedCredentials = btoa(`${state.authInput.username}:${state.authInput.password}`);
    ...
    <div>
      <TabContext value={tabValue}>
          <AppBar position="static" sx={{ marginTop: '20px',backgroundColor: '#FFFFFF' }} elevation={0}>
            ...
            <Tab label={`Headers${state.headerFields.length > 0 ? ` (${state.headerFields.length})` : ''}`} value="1" />
            ...
            <Grid container spacing={2}>
              ...
              {state.headerFields?.map((field, index) => (
                <Grid container item spacing={2} key={index}>
                  ...
                  <Grid item xs={2}>
                    <IconButton onClick={() => setState({...state, headerFields: state.headerFields.filter((_, i) => i !== index)})}>
              ...
              <Grid item xs={12}>
                <Button
                  ...
                  onClick={() => setState({...state, headerFields: [...state.headerFields, { key: '', value: '' }]})}
                  sx={{ marginTop: '10px' }}
                >
                  Add Header
                </Button>
                ...
                <Grid container >
                  <Grid item xs={12} md={7} mb={3}>
                    <TextField
                      ...
                      onChange={(e)=>setState({...state, authorizationType:e.target.value})}
                      variant="outlined"
                    >
                  ...
                  {(state.authorizationType === 'BASIC'|| state.authorizationType==='API_KEY'||state.authorizationType ==='BEARER' ) && (
                    <>
                    <Grid container spacing={2}>
                      ...
                      {(state.authorizationType === 'BASIC' || state.authorizationType==='API_KEY') && (
                      <Grid item xs={12} md={4}>
                        <TextField
                          fullWidth
                          label={state.authorizationType === 'BASIC' ? 'Password' : 'Value'}
                          variant="outlined"
                          type="password"
                          name='password'
                      ...
                      {(state.authorizationType==='API_KEY') && (
                        <Grid item xs={12} md={4}>
                          ...
                          onChange={(e) => setState({...state, addheaderto: e.target.value})}
                          onBlur={addHeaderFields}
                          >
          ...
          <TabPanel value="3">
            ...
            { state.bodyType == 'none' ? <h4>This request does not have a body</h4>:
            <TextField
              fullWidth
              ...
              value={state.body}
              sx={{ marginTop: 2 }}
              placeholder={state.bodyType === 'GraphQL' ? 'Write Your Query Here' : 'Input Body'}
