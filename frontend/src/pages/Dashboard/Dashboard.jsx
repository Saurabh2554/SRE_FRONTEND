...
export default function Dashboard() {
  ...
  const [fetchMetrics, { data: metricsData, error: metricsDataError }] =
    useLazyQuery(GET_ALL_METRICS, {
      errorPolicy: 'all',
    });
  ...
  const handleSearch = (searchParam) => {
    if (businessUnit && subBusinessUnit && dateRange[0] && dateRange[1]) {
      ...
      console.log(
        'Please select business unit, sub-business unit, and date range.'
      );
    }
  };
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12}>
            {businessUnit && subBusinessUnit ? (
              <ApiDataGrid
                metrics={metrics}
                error={metricsDataError ? metricsDataError.message : null}
              />
            ) : (
              <Box
                display="flex"
